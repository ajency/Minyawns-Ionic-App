...
.factory('JobsAPI', ['$q', '$http', 'JobStatus', 'Storage', function($q, $http, JobStatus, Storage){
	...
	JobsAPI.getSingleJob = function(postID){
		var defer = $q.defer();
		
		$http.get(GETURL+'fetchjobs?offset=0&single_job='+postID)
		.then(function(resp, status, headers, config){
			return defer.resolve(resp.data[0]);
	...
	JobsAPI.getMinionDetails = function(applicant, postID){
		var defer = $q.defer();
		
		$http.get(GETURL+'jobminions?minion_id='+applicant+'&job_id='+postID)
		.then(function(resp, status, headers, config){
			return defer.resolve(resp.data[0]);
	...
	JobsAPI.applyUnapplyForJob = function(action, postID){
		var defer = $q.defer();
		var data = { 
    		action: action, 
    		job_id: postID 
